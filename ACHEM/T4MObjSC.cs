...
public class T4MObjSC : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		if (ActiveWind)
		{
			Color value = Wind * Mathf.Sin(GameTime.realtimeSinceServerStartup * WindFrequency);
			value.a = Wind.w;
			Color value2 = Wind * Mathf.Sin(GameTime.realtimeSinceServerStartup * GrassWindFrequency);
			value2.a = Wind.w;
		...
		for (int i = 0; i < ObjPosition.Length; i++)
		{
			...
			if (num <= MaxViewDistance)
			{
				if (num < LOD2Start && ObjLodStatus[i] != 1)
				{
					...
					bool flag2 = (ObjLodScript[i].LOD3.enabled = false);
					lOD.enabled = flag2;
				...
				else if (num >= LOD3Start && ObjLodStatus[i] != 3)
				{
					...
					bool flag2 = (ObjLodScript[i].LOD1.enabled = false);
					lOD3.enabled = flag2;
			...
			else if (ObjLodStatus[i] != 0)
			{
				...
				Renderer lOD5 = ObjLodScript[i].LOD2;
				bool flag6 = (ObjLodScript[i].LOD3.enabled = false);
				bool flag2 = (lOD5.enabled = flag6);
				lOD4.enabled = flag2;
				ObjLodStatus[i] = 0;
			}
		}
