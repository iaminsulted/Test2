...
public class CSDOFScatter : CSPostEffectsBase
{
	...
	private float focalSize = 0.05f;

	public float aperture = 11.5f;

	private Transform focalTransform;

	private float maxBlurSize = 2f;

	private bool highResolution;

	public CSBlurType blurType;
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		Camera component = GetComponent<Camera>();
		focalDistance01 = (focalTransform ? (component.WorldToViewportPoint(focalTransform.position).z / component.farClipPlane) : FocalDistance01(focalLength));
		...
		else if (blurType == CSBlurType.DX11 && (bool)dx11bokehMaterial)
		{
			if (highResolution)
			{
		...
		else
		{
			...
			WriteCoc(source, renderTexture, renderTexture2, fgDilate: true);
			int pass = ((blurSampleCount == CSBlurSampleCount.High || blurSampleCount == CSBlurSampleCount.Medium) ? 17 : 11);
			if (highResolution)
			{
